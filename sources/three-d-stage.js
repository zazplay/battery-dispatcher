  ...
  class ThreeDStage extends HTMLElement {
    ...
    setObject(object) {
      ...
      if (!box.isEmpty()) {
        ...
        const dist =
          (sphere.radius / Math.tan((this._camera.fov * Math.PI) / 360)) * 0.8;
        ...
        const kdir = new THREE.Vector3(4, 7, 5).normalize();
        this._key.position.copy(sphere.center).addScaledVector(kdir, sphere.radius * 2);
        this._key.target.position.copy(sphere.center);
        if (!this._key.target.parent) this._scene.add(this._key.target);
        this._key.shadow.camera.near = sphere.radius * 0.5;
        this._key.shadow.camera.far = sphere.radius * 3.5;
        this._key.shadow.mapSize.set(4096, 4096);
        this._key.shadow.map?.dispose(); this._key.shadow.map = null;
        this._key.shadow.bias = -0.0005;
        this._key.shadow.normalBias = sphere.radius * 0.002;
