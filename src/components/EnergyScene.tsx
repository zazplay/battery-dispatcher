...
import type { LabelId } from '../scene/buildSite';
 ...
 *  three.js and the site model are fetched as their own chunk once the page is up, so the text is interactive before the ~0.7 MB
 *  of scene code arrives; the prerendered page has no scene at all. */
export function EnergyScene() {
  ...
  useEffect(() => {
    ...
    Promise.all([import('../scene/Stage'), import('../scene/buildSite'), import('../scene/alerts')]).then(([{ Stage, isPhone }, { buildSite, animateSite }, { AlertDirector }]) => {
      if (disposed) return;
      let stage: InstanceType<typeof Stage>;
      try {
